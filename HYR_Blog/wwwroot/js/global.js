...
function Success(Title, description, isReload, RedirectUrlForAfter = null) {
    ...
    Swal.fire({
        ...
        if (RedirectUrlForAfter != null && RedirectUrlForAfter != "undefined" && RedirectUrlForAfter != "") {
            location.replace(RedirectUrlForAfter);
        }
    });
}
...
function ErrorAlert(Title, description, isReload, RedirectUrlForAfter = null) {
    if (Title === null || Title === "undefined") {
...
function Warning(Title, description, isReload, RedirectUrlForAfter = null) {
    if (Title === null || Title === "undefined") {
        Title = "مشکلی در عملیات رخ داده است";
    }
    if (description === null || description === "undefined") {
...
function Duplicate(Title, description, isReload, RedirectUrlForAfter = null) {
    if (Title === null || Title === "undefined") {
        Title = "کیس تکراری است";
    }
    if (description === null || description === "undefined") {
        description = "این مورد قبلا ثبت شده است";
    }
    Swal.fire({
        title: Title,
        text: description,
        icon: "ٍError",
        confirmButtonText: "امتحان مجدد"
    }).then((result) => {
        if (isReload === true) {
            location.reload();
        }
...
function NotFound(Title, description, isReload, RedirectUrlForAfter = null) {
    if (Title === null || Title === "undefined") {
        Title = "یافت نشد";
    }
    if (description === null || description === "undefined") {
        description = "کیس مورد نظر یافت نشد";
    }
    Swal.fire({
        title: Title,
        text: description,
...
$(document).ready(function () {
    ...
    //loadDataTable();
    GetCartWithAjax();
    var result = getCookie("SweetAlert");
    if (result) {
        result = JSON.parse(result);
        if (result.StatusCode === 0) {
            Success(result.Title, result.StatusMessage, result.BeReload, result.RedirectUrlForAfter);
        } else if (result.StatusCode === 1) {
            ErrorAlert(result.Title, result.StatusMessage, result.BeReload, result.RedirectUrlForAfter);
        } else if (result.StatusCode === 2) {
            NotFound(result.Title, result.StatusMessage, result.BeReload, result.RedirectUrlForAfter);
        } else if (result.StatusCode === 3) {
            Duplicate(result.Title, result.StatusMessage, result.BeReload, result.RedirectUrlForAfter);
        }
        deleteCookie("SweetAlert");
    }
...
function DeleteCaseWithAjax(Url, method, data) {
    Swal.fire({
        title: "هشدار !!",
        text: "آیا از حذف اطمینان دارید ؟",
        icon: "warning",
...
function GetCartWithAjax() {
    $.ajax({
        url: "/Index/GetOrUpdateCart",
        type: "post",
        beforeSend: function (xhr) {
            xhr.setRequestHeader("XSRF-TOKEN", $('input:hidden[name="__RequestVerificationToken"]').val());
        },
